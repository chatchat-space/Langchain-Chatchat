...
class PlatformToolsMessageChunk(AIMessage, BaseMessageChunk):
    """Message chunk from an AI."""

    # Ignoring mypy re-assignment here since we're overriding the value
    # to make sure that the chunk variant can be discriminated from the
    # non-chunk variant.
    type: Literal["PlatformToolsMessageChunk"] = "PlatformToolsMessageChunk"  # type: ignore[assignment] # noqa: E501
        ...
        if not values["tool_call_chunks"]:
            ...
            if values["invalid_tool_calls"]:
                ...
                tool_call_chunks.extend(
                    [
                        InvalidToolCall(
                            name=tc["name"], args=tc["args"], id=tc.get("id"), error=None
                        )
