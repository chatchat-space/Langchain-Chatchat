...
const Logo = memo<LogoProps>(
  ({ type = 'combine', size = 32, style, extra, className, imageUrl, localImage, ...rest  }) => {
    ...
    switch (type) {
      case 'flat': {
        logoComponent = (
          <img alt="chatchat" height={size} src={`${process.env.PUBLIC_URL}/images/chathead.webp`} style={style} width={size} />
        );
        break;
      }
      case '3d': {
      ...
      case 'combine': {
        // logoComponent = <div className={`logo ${type}`}>{/* Render logo based on type */}</div>;
        logoComponent = (
          <>
            <img alt="chatchat" height={size} src={`${process.env.PUBLIC_URL}/images/chathead.webp`} style={style} width={size} />
