...
class CustomAsyncIteratorCallbackHandler(AsyncIteratorCallbackHandler):
    ...
    async def on_tool_start(self, serialized: Dict[str, Any], input_str: str, *, run_id: UUID,
        ...
        print(f"on_tool_start: {self.cur_tool['input_str']}")
    ...
    async def on_agent_finish(
            self,finish: AgentFinish,*,run_id: UUID,parent_run_id: Optional[UUID] = None,tags: Optional[List[str]] = None,
            **kwargs: Any,
    ) -> None:

        self.cur_tool.update(
            status=Status.running,
            llm_token=finish.return_values["output"],
        )
        self.queue.put_nowait(dumps(self.cur_tool))
        self.cur_tool = {}